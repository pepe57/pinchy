...
| `channel.restarted`      | The watchdog restarted a channel account stuck in a Telegram `getUpdates` 409 conflict, so polling resumes as soon as the conflicting poller goes away instead of waiting out OpenClaw's internal retry backoff. Fires when the conflict crosses into `channel.polling_failed`, then at increasing intervals while it persists — one row per attempt, success or failure. Detail records `channel`, `account.{id,name}`, `reason`, `lastError`, `restartAttempt`, and an `error` on failure. Actor is `system` / `channel-watchdog`. Disable with `TELEGRAM_CONFLICT_AUTO_RESTART=false`. |
