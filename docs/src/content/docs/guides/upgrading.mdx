...
- The same watchdog also covers a **run that OpenClaw never acknowledges** — the dispatch hangs at request-receive (the failure mode behind "I sent a message and nothing happened, no error"). The run is registered at dispatch time, and if no first chunk arrives within a **first-chunk timeout** (default 180 seconds — kept above the 150s dispatch-race retry budget so a normal OpenClaw restart isn't mistaken for a hang), the watchdog aborts it and broadcasts a **retryable** error so the user can resend instead of staring at an indefinitely blank thread. This is audited as `chat.run_no_first_chunk` — distinct from `chat.run_timed_out` so "the agent never started responding" is separable from "the agent ran long".
